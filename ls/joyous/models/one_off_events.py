        ...
        class ByDayIterable(ModelIterable):
            def __iter__(self):
                ...
                for page in super().__iter__():
                    ...
                    pageToDate = getLocalDate(page.date, page.time_to, page.tz)
    ...
    def byDay(self, fromDate, toDate):
        ...
        class ByDayIterable(ModelIterable):
            def __iter__(self):
                evods = EventsByDayList(fromDate, toDate)
                for page in super().__iter__():
                    pageFromDate = getLocalDate(page.date_from, page.time_from, page.tz)
                    pageToDate = getLocalDate(page.date_to, page.time_to, page.tz)
                    thisEvent = ThisEvent(page, url=page.get_url(request))
                    evods.add(thisEvent, pageFromDate, pageToDate)
                yield from evods
        ...
        qs = self._clone()
        qs._iterable_class = ByDayIterable
        return qs.filter(date_to__gte=fromDate - _2days).filter(
            date_from__lte=toDate + _2days
        )
