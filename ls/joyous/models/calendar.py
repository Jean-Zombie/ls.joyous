...
class CalendarPageForm(BorgPageForm):
    ...
    def registerImportHandler(cls, handler):
        ...
        uploadWidget = forms.FileInput(
            attrs={"accept": "text/calendar," "application/zip," ".ics,.zip"}
        )
        cls.declared_fields["upload"] = forms.FileField(
            label=_("Upload"), required=False, widget=uploadWidget
        )
        cls.declared_fields["utc2local"] = forms.BooleanField(
            label=_("Convert UTC to localtime?"), required=False, initial=True
        )
        CalendarPage.settings_panels.append(
            Panel(
                [
                    FieldPanel("upload"),
...
class CalendarPage(RoutablePageMixin, Page, metaclass=FormDefender):
    ...
    intro = RichTextField(_("intro"), blank=True, help_text=_("Introductory text."))
    view_choices = MultipleSelectField(
        _("view choices"),
        blank=True,
        default=["L", "W", "M"],
    ...
    default_view = models.CharField(
        _("default view"), default="M", max_length=15, choices=EVENTS_VIEW_CHOICES
    )
    ...
    settings_panels = Page.settings_panels + [
        MultiFieldPanel(
            [FieldPanel("view_choices"), FieldPanel("default_view")],
    ...
    def serveMonth(self, request, year=None, month=None):
        ...
        def myUrl(urlYear, urlMonth):
            if 1900 <= urlYear <= 2099:
                return myurl + self.reverse_subpage(
                    "serveMonth", args=[urlYear, urlMonth]
                )
        ...
        weeklyUrl = myurl + self.reverse_subpage("serveWeek", args=[weekYear, weekNum])
    ...
    def serveUpcoming(self, request):
        ...
        return TemplateResponse(request, "joyous/calendar_list_upcoming.html", cxt)
    ...
    def serveMiniMonth(self, request, year=None, month=None):
        ...
        return TemplateResponse(request, "joyous/includes/minicalendar.html", cxt)
    ...
    def _getEventsByDay(self, request, firstDay, lastDay):
        ...
        return getAllEventsByDay(
            request, firstDay, lastDay, home=home, holidays=self.holidays
        )
    ...
    def _getEventsByWeek(self, request, year, month):
        ...
        """
        home = Site.find_for_request(request).root_page
        return getAllEventsByWeek(
            request, year, month, home=home, holidays=self.holidays
        )
...
class SpecificCalendarPage(ProxyPageMixin, CalendarPage):
    ...
    def _getEventsByDay(self, request, firstDay, lastDay):
        ...
        """
        return getAllEventsByDay(
            request, firstDay, lastDay, home=self, holidays=self.holidays
        )

    def _getEventsByWeek(self, request, year, month):
        """Return my child events for the given month grouped by week."""
        return getAllEventsByWeek(
            request, year, month, home=self, holidays=self.holidays
        )
...
class GeneralCalendarPage(ProxyPageMixin, CalendarPage):
    ...
    def _getEventsByDay(self, request, firstDay, lastDay):
        ...
        """
        return getAllEventsByDay(request, firstDay, lastDay, holidays=self.holidays)
