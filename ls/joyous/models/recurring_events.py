        ...
        class ByDayIterable(ModelIterable):
            def __iter__(self):
                ...
                for page in super().__iter__():
                    ...
                    for occurence in page.repeat.between(
                        fromDate - startDelta, toDate + _2days, inc=True
                    ):
                        thisEvent = None
                        ...
                        if exception:
                            ...
                            if (
                                closedHols.isAuthorized(request)
                                and closedHols.cancellation_title
                            ):
                                thisEvent = ThisEvent(
                        ...
                        if thisEvent:
                            pageFromDate = getLocalDate(
                                occurence, page.time_from, page.tz
                            )
                            daysDelta = dt.timedelta(days=page.num_days - 1)
                            pageToDate = getLocalDate(
                                occurence + daysDelta, page.time_to, page.tz
                            )
            ...
            def __getExceptionsFor(self, page):
                ...
                for extraInfo in (
                    ...
                    exceptions[exceptDate] = ThisEvent(
                        title, extraInfo, extraInfo.get_url(request)
                    )
                for cancellation in CancellationPage.events.child_of(page).filter(
                    except_date__range=dateRange
                ):
                ...
                for shutdown in (
                    ExtCancellationPage.events.child_of(page)
                    .filter(cancelled_from_date__lte=dateRange[1])
                    .filter(
                        Q(cancelled_to_date__gte=dateRange[0])
            ...
            def __getClosedForHolidays(self, page):
                closedHols = (
                    ClosedForHolidaysPage.events.hols(holidays).child_of(page).first()
                )
                return closedHols
...
class RecurringEventPage(EventBase, Page, metaclass=FormDefender):
    ...
    def next_date(self):
        ...
        if myNextDt is not None:
            return getLocalDate(myNextDt.date(), self.time_from, self.tz, dt.time.min)
    ...
    def _current_datetime_from(self):
        ...
        timeDelta = dt.timedelta(
            days=self.num_days - 1,
            hours=timeTo.hour - timeFrom.hour,
            minutes=timeTo.minute - timeFrom.minute,
            seconds=timeTo.second - timeFrom.second,
        )
        myNextDt = self.__after(
            myNow - timeDelta, excludeCancellations=True, excludeExtraInfo=True
        )
        if myNextDt is not None:
            return getLocalDatetime(
                myNextDt.date(), self.time_from, self.tz, dt.time.max
            )
    ...
    def _future_datetime_from(self):
        ...
        """
        myNextDt = self.__after(
            timezone.localtime(timezone=self.tz),
    ...
    def prev_date(self):
        ...
        if myPrevDt is not None:
            return getLocalDate(myPrevDt.date(), self.time_from, self.tz, dt.time.min)
    ...
    def _past_datetime_from(self):
        ...
        if myPrevDt is not None:
            return getLocalDatetime(
                myPrevDt.date(), self.time_from, self.tz, dt.time.max
            )
    ...
    def status(self):
        ...
        eventFinish = getAwareDatetime(
            eventStart.date() + eventDaysDelta, event.time_to, self.tz
        )
        ...
        if (
            self.repeat.until
            and eventFinish < myNow
            and self.__afterOrPostponedTo(myNow)[0] is None
        ):
    ...
    def when(self):
        ...
        if myFromDt is not None:
            ...
            dateFrom, timeFrom = getLocalDateAndTime(
                myFromDt.date(), self.time_from, self.tz, dt.time.min
            )
            daysDelta = dt.timedelta(days=self.num_days - 1)
            dateTo, timeTo = getLocalDateAndTime(
                myFromDt.date() + daysDelta, self.time_to, self.tz
            )
        if dateFrom == dateTo:
    ...
    def _futureExceptions(self, request):
        ...
        """
        retval = []
        for extraInfo in ExtraInfoPage.events(request).child_of(self).future():
            retval.append(extraInfo)
        for cancellation in CancellationPage.events(request).child_of(self).future():
            postponement = getattr(cancellation, "postponementpage", None)
            if postponement:
                retval.append(postponement)
            else:
                retval.append(cancellation)
        for shutdown in ExtCancellationPage.events(request).child_of(self).future():
            retval.append(shutdown)
        closedHols = (
            ClosedForHolidaysPage.events(request, self.holidays).child_of(self).first()
        )
    ...
    def _nextOn(self, request):
        ...
        if myNextDt is not None:
            nextDate, nextTime = getLocalDateAndTime(
                myNextDt.date(), event.time_from, self.tz, dt.time.min
            )
            retval = "{} {}".format(
                dateFormat(nextDate), timeFormat(nextTime, prefix=gettext("at "))
            )
            if event is not self and event.isAuthorized(request):
                retval = format_html(
                    '<a class="inline-link" href="{}">{}</a>', event.url, retval
                )
        return retval
    ...
    def _occursOn(self, myDate):
        ...
        if myDate not in self.repeat:
            return False
        if CancellationPage.events.child_of(self).filter(except_date=myDate).exists():
            return False
        if (
            ExtCancellationPage.events.child_of(self)
            .filter(cancelled_from_date__lte=myDate)
            .filter(
                Q(cancelled_to_date__gte=myDate) | Q(cancelled_to_date__isnull=True)
            )
            .exists()
        ):
            return False
        closedHols = (
            ClosedForHolidaysPage.events.hols(self.holidays).child_of(self).first()
        )
    ...
    def _getMyFirstDatetimeFrom(self):
        ...
        """
        myStartDt = getAwareDatetime(self.repeat.dtstart, None, self.tz, dt.time.min)
    ...
    def _getMyFirstDatetimeTo(self, myFirstDt=False):
        ...
        if myFirstDt is not None:
            daysDelta = dt.timedelta(days=self.num_days - 1)
            return getAwareDatetime(
                myFirstDt.date() + daysDelta, self.time_to, self.tz, dt.time.max
            )
    ...
    def __afterOrPostponedTo(self, fromDt):
        ...
        if after:
            ...
            postponements = (
                PostponementPage.events.child_of(self)
                .filter(date__range=(dateRange))
                .order_by("date", "time_from")
            )
            for postponement in postponements:
                postDt = getAwareDatetime(
                    postponement.date, postponement.time_from, self.tz, dt.time.min
                )
                postDtMax = getAwareDatetime(
                    postponement.date, postponement.time_from, self.tz, dt.time.max
                )
    ...
    def __after(self, fromDt, excludeCancellations=True, excludeExtraInfo=False):
        ...
        if excludeCancellations:
            for cancelled in CancellationPage.events.child_of(self).filter(
                except_date__gte=fromDate
            ):
                exceptions.add(cancelled.except_date)
            # TODO consider storing extended cancellations in an interval tree?
            shutdowns = ExtCancellationPage.events.child_of(self).filter(
                Q(cancelled_to_date__gte=fromDate) | Q(cancelled_to_date__isnull=True)
            )
            closedHols = (
                ClosedForHolidaysPage.events.hols(self.holidays).child_of(self).first()
            )
        if excludeExtraInfo:
            for info in (
                ExtraInfoPage.events.child_of(self)
                .filter(except_date__gte=fromDate)
                .exclude(extra_title="")
            ):
                exceptions.add(info.except_date)
        for occurence in self.repeat.xafter(
            fromDate, count=self.MAX_REPEAT_COUNT, inc=True
        ):
            if occurence in exceptions:
                continue
            shutdown = next(
                (shutdown for shutdown in shutdowns if shutdown._closedOn(occurence)),
                None,
            )
            ...
            if closedHols and closedHols._closedOn(occurence):
                continue
            return getAwareDatetime(occurence, self.time_from, self.tz, dt.time.min)
    ...
    def __before(self, fromDt, excludeCancellations=True, excludeExtraInfo=False):
        ...
        if excludeCancellations:
            for cancelled in CancellationPage.events.child_of(self).filter(
                except_date__lte=fromDate
            ):
                exceptions.add(cancelled.except_date)
            shutdowns = ExtCancellationPage.events.child_of(self).filter(
                cancelled_from_date__lte=fromDate
            )
            closedHols = (
                ClosedForHolidaysPage.events.hols(self.holidays).child_of(self).first()
            )
...
class MultidayRecurringEventPage(ProxyPageMixin, RecurringEventPage):
    ...
    content_panels = (
        RecurringEventPage.content_panels0
        + [
            FieldPanel("num_days"),
        ]
...
class DateExceptionPageForm(BorgPageForm):
    def _checkSlugAvailable(self, cleaned_data, slugName=None):
        ...
        if not Page._slug_is_available(slug, self.parent_page, self.instance):
            self.add_error(
                "except_date", _("That date already has") + " " + description.lower()
            )
...
class DateExceptionBase(EventExceptionBase):
    ...
    def local_title(self):
        ...
        title = _("{exception} for {date}").format(
            exception=_(name), date=dateFormat(exceptDate)
        )
        return title
    ...
    def _getLocalWhen(self, date_from, num_days=1):
        ...
        if num_days > 1 or self.time_to is not None:
            daysDelta = dt.timedelta(days=self.num_days - 1)
            dateTo, timeTo = getLocalDateAndTime(
                date_from + daysDelta, self.time_to, self.tz
            )
        else:
            dateTo = dateFrom
            timeTo = None
...
class ExtraInfoQuerySet(DateExceptionQuerySet):
    def this(self):
        ...
        class ThisIterable(ModelIterable):
            def __iter__(self):
                for page in super().__iter__():
                    yield ThisEvent(page.extra_title, page, page.get_url(request))
    ...
    def this(self):
        ...
        class ThisIterable(ModelIterable):
            def __iter__(self):
                for page in super().__iter__():
                    yield ThisEvent(
                        page.cancellation_title, page, page.getCancellationUrl(request)
                    )
...
class CancellationBase(models.Model):
    ...
    cancellation_title.help_text = _(
    ...
    def this(self):
        ...
        class ThisIterable(ModelIterable):
            def __iter__(self):
                for page in super().__iter__():
                    yield ThisEvent(
                        page.postponement_title, page, page.get_url(request)
                    )
        ...
        class ByDayIterable(ModelIterable):
            def __iter__(self):
                ...
                for page in super().__iter__():
                    ...
                    pageFromDate = getLocalDate(page.date, page.time_from, page.tz)
                    daysDelta = dt.timedelta(days=page.num_days - 1)
                    pageToDate = getLocalDate(
                        page.date + daysDelta, page.time_to, page.tz
                    )
                    evods.add(thisEvent, pageFromDate, pageToDate)
                yield from evods
...
class PostponementPage(RoutablePageMixin, RescheduleEventBase, CancellationPage):
    ...
    def serveCancellation(self, request):
        return TemplateResponse(
            request, "joyous/postponement_page_from.html", self.get_context(request)
        )
    ...
    def postponed_from_when(self):
        ...
        what = self.what
        if what:
            return _("{what} from {when}").format(
                what=what, when=self.cancellationpage.when
            )
    ...
    def what(self):
        ...
        """
        originalFromDt = dt.datetime.combine(
            self.except_date, getTimeFrom(self.overrides.time_from)
        )
        ...
        originalToDt = dt.datetime.combine(
            self.except_date + originalDaysDelta, getTimeTo(self.overrides.time_to)
        )
        changedDaysDelta = dt.timedelta(days=self.num_days - 1)
        changedToDt = dt.datetime.combine(
            self.except_date + changedDaysDelta, getTimeTo(self.time_to)
        )
    ...
    def this(self):
        ...
        class ThisIterable(ModelIterable):
            def __iter__(self):
                for page in super().__iter__():
                    yield ThisEvent(
                        page.cancellation_title, page, page.get_url(request)
                    )
...
class ClosedForHolidaysPageForm(BorgPageForm):
    ...
    closed_for = forms.MultipleChoiceField(
        label=_("These holidays"),
        required=False,
        widget=FilteredSelectMultiple(_("Holidays"), is_stacked=False),
    )
...
class ClosedForHolidaysPage(
    CancellationBase, EventExceptionBase, Page, metaclass=FormDefender
):
    class Meta:
    ...
    content_panels = [
        PageChooserPanel("overrides"),
        MultiFieldPanel(
            [FieldPanel("all_holidays"), FieldPanel("closed_for")],
    ...
    def can_create_at(cls, parent):
        retval = (
            ...
            and not cls.objects.descendant_of(parent).exists()
        )
        return retval
    ...
    def __after(self, fromDt, excludeCancellations=True):
        ...
        shutdowns = []
        if excludeCancellations:
            for cancelled in CancellationPage.events.child_of(self.overrides).filter(
                except_date__gte=fromDate
            ):
                exceptions.add(cancelled.except_date)
            shutdowns = ExtCancellationPage.events.child_of(self.overrides).filter(
                Q(cancelled_to_date__gte=fromDate) | Q(cancelled_to_date__isnull=True)
            )
...
class ExtCancellationQuerySet(EventQuerySet):
    def current(self):
        qs = super().current()
        return qs.filter(
            Q(cancelled_to_date__gte=todayUtc() - _1day)
            | Q(cancelled_to_date__isnull=True)
        )
...
class ExtCancellationPage(
    ...
    def local_title(self):
        ...
        title = _("Cancellation from {fromDate} {untilWhen}").format(
            fromDate=dateFormat(self.cancelled_from_date), untilWhen=self.until_when
        )
        return title
    ...
    def full_clean(self, *args, **kwargs):
        ...
        with translation.override("en"):
            ...
            self.slug = "{}-{}-cancellation".format(self.cancelled_from_date, slugTo)
        super().full_clean(*args, **kwargs)
    ...
    def _getMyDates(self, fromDate=dt.date.min, toDate=FAR_DATE):
        ...
        if self.cancelled_to_date is not None and self.cancelled_to_date < toDate:
    ...
    def __after(self, fromDt):
        ...
        if self.cancelled_to_date is None or occurence <= self.cancelled_to_date:
            return getAwareDatetime(occurence, self.time_from, self.tz, dt.time.min)
    ...
    def __before(self, fromDt):
        ...
        if self.time_from and self.time_from > fromDt.time():
            fromDate -= _1day
        if self.cancelled_to_date is not None and fromDate > self.cancelled_to_date:
