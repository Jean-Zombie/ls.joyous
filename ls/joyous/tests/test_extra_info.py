...
class Test(TestCase):
    def setUp(self):
        ...
        self.event = RecurringEventPage(
            ...
            title="Test Meeting",
            repeat=Recurrence(
                dtstart=dt.date(1988, 1, 1), freq=WEEKLY, byweekday=[MO, WE, FR]
            ),
    ...
    def testGetEventsByDay(self):
        events = RecurringEventPage.events.byDay(
            dt.date(1988, 11, 1), dt.date(1988, 11, 30)
        )
    ...
    def testWhen(self):
        self.assertEqual(
            self.info.when, "Friday 11th of November 1988 at 1pm to 3:30pm"
        )
    ...
    def testPastDt(self):
        self.assertEqual(self.info._past_datetime_from, datetimetz(1988, 11, 11, 13, 0))
...
class TestMultiday(TestCase):
    def setUp(self):
        self.home = Page.objects.get(slug="home")
        self.user = User.objects.create_user("i", "i@bar.test", "s3(r3t")
        self.calendar = CalendarPage(owner=self.user, slug="schedule", title="Schedule")
        self.home.add_child(instance=self.calendar)
        self.calendar.save_revision().publish()
    ...
    def testCurrentDt(self):
        self.assertEqual(
            self.info._current_datetime_from, datetimetz(2018, 2, 12, 10, 0)
        )
    ...
    def testPastDt(self):
        self.assertEqual(self.info._past_datetime_from, datetimetz(2018, 2, 12, 10, 0))
...
class TestPageForm(TestCase):
    ...
    def testExceptionAlreadyExists(self):
        ...
        self.assertDictEqual(
            form.errors, {"except_date": ["That date already has extra information"]}
        )
...
class TestPageFormDeutsche(TestCase):
    ...
    def setUp(self):
        ...
        self.calendar = CalendarPage(
            owner=self.user, slug="ereignisse", title="Ereignisse"
        )
        self.home.add_child(instance=self.calendar)
        self.calendar.save_revision().publish()
        self.event = RecurringEventPage(
