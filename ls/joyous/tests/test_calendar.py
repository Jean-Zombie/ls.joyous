...
class TestCalendar(TestCase):
    def setUp(self):
        ...
        calendar = CalendarPage(
            owner=self.user, slug="events", title="Events", default_view="L"
        )
    ...
    def testMonthView(self):
        ...
        month = select("tr.joy-cal__headings th.joy-cal__heading .joy-cal__month-name")[
            0
        ]
        ...
        holidayNames = [
            holiday.string.strip()
            for holiday in select("tbody td.joy-cal__day .joy-cal__holiday-name")
        ]
        self.assertEqual(
            holidayNames, ["Taranaki Anniversary Day", "Otago Anniversary Day"]
        )
    ...
    def testWeekView(self):
        ...
        self.assertEqual(holidayNames[0].string.strip(), "Taranaki Anniversary Day")
    ...
    def testPastEvents(self):
        ...
        self.assertEqual(
            when.string.strip(), "Sunday 5th of June 2011 at 9:30am to 11am"
        )
    ...
    def testMiniMonthAjaxView(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            len(select("table.joy-minical thead tr .joy-minical__weekday--sun")), 1
        )
...
class TestCalendarPageForm(TestCase):
    ...
    def testSave(self):
        ...
        form.save()
        handler.load.assert_called_with(self.page, self.request, "FILE", utc2local=True)
...
class TestFrançais(TestCase):
    def setUp(self):
        ...
        self.user = User.objects.create_superuser("i", "i@j.test", "s3(r3t")
        calendar = CalendarPage(owner=self.user, slug="calendrier", title="Calendrier")
        Page.objects.get(slug="home").add_child(instance=calendar)
        calendar.save_revision().publish()
        event = SimpleEventPage(
            owner=self.user,
    ...
    def testWeekView(self):
        ...
        self.assertEqual(holidayName[0].string.strip(), "Taranaki Anniversary Day")
...
class TestSpecificCalendar(TestCase):
    def setUp(self):
        ...
        self.request.user = self.user
        self.request.session = {}
        self.calendar1 = SpecificCalendarPage(
            owner=self.user, slug="calendar1", title="Red Team Calendar"
        )
        home = getPage("/home/")
        ...
        event.save_revision().publish()
        self.calendar2 = SpecificCalendarPage(
            owner=self.user, slug="calendar2", title="Green Team Calendar"
        )
    ...
    def testEventsByDay(self):
        evods = self.calendar1._getEventsByDay(
            self.request, dt.date(2011, 7, 1), dt.date(2011, 7, 10)
        )
    ...
    def testGetEventFromUid(self):
        event = self.calendar1._getEventFromUid(
            self.request, "570ed9c4-4503-4b45-b15e-c99faed9c531"
        )
        self.assertEqual(event.title, "Football Game")
        event = self.calendar1._getEventFromUid(
            self.request, "eb50e787-12bf-477b-8493-c4414ac001ca"
        )
...
class TestGeneralCalendar(TestCase):
    def setUp(self):
        ...
        self.request.user = self.user
        self.request.session = {}
        self.calendar = GeneralCalendarPage(
            owner=self.user, slug="calendar", title="My Calendar"
        )
        home = getPage("/home/")
    ...
    def testEventsByDay(self):
        evods = self.calendar._getEventsByDay(
            self.request, dt.date(2011, 8, 1), dt.date(2011, 8, 20)
        )
...
class TestMultiCalendarCreate(TestCase):
    def setUp(self):
        ...
        Site.objects.create(
            hostname="nova.joy.test", root_page_id=self.sub.id, is_default_site=False
        )
        SpecificCalendarPage.is_creatable = True
        GeneralCalendarPage.is_creatable = True
    ...
    def testMainSiteAnotherSpecificCalendar(self):
        calendar = SpecificCalendarPage(owner=self.user, slug="events", title="Events")
        self.main.add_child(instance=calendar)
        calendar.save_revision().publish()
        self.assertTrue(SpecificCalendarPage.can_create_at(self.main))

    def testMainSiteAnotherGeneralCalendar(self):
        calendar = GeneralCalendarPage(owner=self.user, slug="events", title="Events")
        self.main.add_child(instance=calendar)
        calendar.save_revision().publish()
    ...
    def testCalendarMixture(self):
        general = GeneralCalendarPage(owner=self.user, slug="events1", title="Events")
        self.main.add_child(instance=general)
        general.save_revision().publish()
        self.assertTrue(CalendarPage.can_create_at(self.main))
        calendar = CalendarPage(owner=self.user, slug="events2", title="Events")
        self.main.add_child(instance=calendar)
        calendar.save_revision().publish()
        self.assertTrue(SpecificCalendarPage.can_create_at(self.main))
        specific = SpecificCalendarPage(owner=self.user, slug="events3", title="Events")
