...
class TestMultiSite(TestCase):
    ...
    def _setUpMainSite(self):
        ...
        event = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(1984, 8, 5), freq=WEEKLY, byweekday=[MO, WE, FR]
            ),
        ...
        event = RecurringEventPage(
            ...
            title="Drama Group",
            repeat=Recurrence(
                dtstart=dt.date(1984, 8, 14), freq=WEEKLY, byweekday=[TH], interval=2
            ),
    ...
    def _setUpNovaSubsite(self):
        ...
        Site.objects.create(
            hostname="nova.joy.test", root_page_id=home.id, is_default_site=False
        )
        events = CalendarPage(owner=self.user, slug="nova-events", title="Nova Events")
        home.add_child(instance=events)
        events.save_revision().publish()
        committee = RecurringEventPage(
            owner=self.user,
            slug="executive-meeting",
            title="Executive Committee Meeting",
            repeat=Recurrence(dtstart=dt.date(1984, 8, 5), freq=WEEKLY, byweekday=[TH]),
        ...
        cancellation = CancellationPage(
            ...
            cancellation_details="The committee will be at " "the working bee",
        )
    ...
    def _setUpVeterisSubsite(self):
        ...
        activities.save_revision().publish()
        Site.objects.create(
            hostname="veteris.joy.test", root_page_id=home.id, is_default_site=False
        )
        events = CalendarPage(
            owner=self.user, slug="veteris-events", title="Veteris Events"
        )
        home.add_child(instance=events)
        events.save_revision().publish()
        committee = RecurringEventPage(
    ...
    def _getContext(self, hostname=None, slug="", **kwargs):
        ...
        vars = {"request": self._getRequest(site, slug), "page": getPage("/home/")}
        vars.update(kwargs)
    ...
    def testEventsThisWeek(self):
        out = Template("{% load joyous_tags %}" "{% events_this_week %}").render(
            self._getContext()
        )
...
""",
    ...
    def testSubsiteEventsThisWeek(self):
        out = Template("{% load joyous_tags %}" "{% events_this_week %}").render(
            self._getContext(
                hostname="nova.joy.test", page=getPage("/home/nova/activities/")
            )
        )
        self.assertHTMLEqual(
            out,
...
""",
    ...
    def testMinicalendar(self):
        out = Template("{% load joyous_tags %}" "{% minicalendar %}").render(
            self._getContext()
        )
        ...
        self.assertEqual(
            len(select('tbody td.joy-minical__day a[title="Lunchtime Chess Matches"]')),
            12,
        )
        ...
        self.assertEqual(
            links[1]["title"], "Early Morning Matches, Executive Committee Meeting"
        )
    ...
    def testAllUpcomingEvents(self):
        out = Template("{% load joyous_tags %}" "{% all_upcoming_events %}").render(
            self._getContext()
        )
        soup = BeautifulSoup(out, "html5lib")
        ...
        self.assertEqual(
            novaexec.find(class_="joy-ev-next-on").get_text(strip=True),
        ...
        self.assertEqual(
            blitz.find(class_="joy-ev-when").get_text(strip=True),
            "Thursday 13th of September at 12:30pm to 5pm",
        )
        self.assertEqual(
            drama.find(class_="joy-ev-when").get_text(strip=True),
            "Fortnightly on Thursdays at 5pm",
        )
        self.assertEqual(
            drama.find(class_="joy-ev-next-on").get_text(strip=True),
            "Next on Thursday 13th of September at 5pm",
        )
        self.assertEqual(
            lecture.find(class_="joy-ev-when").get_text(strip=True),
            "Friday 14th of September at 7pm",
        )
        self.assertEqual(
            lecture.find(class_="joy-ev-where").get_text(strip=True),
            "Lecture Hall C[map]",
        )
        self.assertEqual(market.a["href"], "/events/flea-market/")
        self.assertEqual(market.a.get_text(strip=True), "Flea Market")
        self.assertEqual(
            chess2.a["href"], "/chess-club/lunchtime-matches/1984-10-03-postponement/"
        )
        self.assertEqual(
            chess2.find(class_="joy-ev-from-when").get_text(strip=True),
    ...
    def testSubsiteUpcomingEvents(self):
        out = Template("{% load joyous_tags %}" "{% subsite_upcoming_events %}").render(
            self._getContext(
                hostname="nova.joy.test", page=getPage("/home/nova/activities/")
            )
        )
        ...
        self.assertEqual(novaexec.a.get_text(strip=True), "Executive Committee Meeting")
        self.assertEqual(blitz.a.get_text(strip=True), "Rubbish Blitz")
        self.assertEqual(cancel.a.get_text(strip=True), "Meeting Cancelled")

    @freeze_timetz("1984-09-15 10:00")
    def testGroupUpcomingEvents(self):
        out = Template("{% load joyous_tags %}" "{% group_upcoming_events %}").render(
            self._getContext(page=getPage("/home/chess-club/"))
        )
        self.assertHTMLEqual(
            out,
            """
...
""",
    ...
    def testGroupUpcomingEventsNotGroup(self):
        out = Template("{% load joyous_tags %}" "{% group_upcoming_events %}").render(
            self._getContext(page=getPage("/home/"))
        )
        self.assertHTMLEqual(out, "")

    def testGroupUpcomingEventsNoPage(self):
        out = Template("{% load joyous_tags %}" "{% group_upcoming_events %}").render(
            self._getContext(page=None)
        )
        self.assertHTMLEqual(out, "")

    @freeze_timetz("1984-08-04 10:00")
    def testFutureExceptionsImplicitPage(self):
        out = Template("{% load joyous_tags %}" "{% future_exceptions %}").render(
            self._getContext(
                hostname="nova.joy.test",
        ...
        self.assertEqual(
            cancellation.find(class_="joy-field").get_text(strip=True),
            "Meeting Cancelled",
        )
    ...
    def testFutureExceptionsExplicitPage(self):
        out = Template(
            "{% load joyous_tags %}" "{% future_exceptions kasparov %}"
        ).render(
            self._getContext(kasparov=getPage("/home/chess-club/lunchtime-matches/"))
        )
        soup = BeautifulSoup(out, "html5lib")
        items = soup(class_="joy-ev-related__item")
        ...
        self.assertEqual(
            postponement.find(class_="joy-field").get_text(strip=True),
    ...
    def testNextOn(self):
        out = Template("{% load joyous_tags %}" "{% next_on %}").render(
            self._getContext(
                hostname="nova.joy.test",
                page=getPage("/home/nova/nova-events/executive-meeting/"),
            )
    ...
    def testLocationGMap(self):
        ...
        out = Template("{% load joyous_tags %}" "{% location_gmap location %}").render(
            self._getContext(page=page, location=page.location)
        )
        self.assertHTMLEqual(
            out,
            """
...
""",
    ...
    def testTimeDisplay(self):
        out = Template("{% load joyous_tags %}" "{{ alarm | time_display }}").render(
            self._getContext(alarm=dt.time(6, 55))
        )
        self.assertEqual(out, "6:55am")

    def testAtTimeDisplay(self):
        out = Template(
            "{% load joyous_tags %}" "{{ meeting | at_time_display }}"
        ).render(self._getContext(meeting=dt.time(8, 30)))
        self.assertEqual(out, "at 8:30am")

    def testDateDisplay(self):
        out = Template("{% load joyous_tags %}" "{{ day | date_display }}").render(
            self._getContext(day=dt.date(2008, 3, 10))
        )
