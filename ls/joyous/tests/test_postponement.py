...
class Test(TestCase):
    def setUp(self):
        ...
        self.event = RecurringEventPage(
            ...
            title="Test Meeting",
            repeat=Recurrence(
                dtstart=dt.date(1990, 1, 1), freq=WEEKLY, byweekday=[MO, WE, FR]
            ),
        ...
        self.postponement = PostponementPage(
            ...
            cancellation_details="The meeting has been postponed until tomorrow",
    ...
    def testGetEventsByDay(self):
        events = RecurringEventPage.events.byDay(
            dt.date(1990, 10, 1), dt.date(1990, 10, 31)
        )
        ...
        events = PostponementPage.events.byDay(
            dt.date(1990, 10, 1), dt.date(1990, 10, 31)
        )
        self.assertEqual(len(events), 31)
        evod = events[10]
    ...
    def testWhen(self):
        self.assertEqual(
            self.postponement.when, "Thursday 11th of October 1990 at 1pm to 4:30pm"
        )
    ...
    def testPostponedFrom(self):
        self.assertEqual(
            self.postponement.postponed_from, "Wednesday 10th of October 1990"
        )

    def testPostponedTo(self):
        self.assertEqual(
            self.postponement.postponed_to, "Thursday 11th of October 1990"
        )
    ...
    def testPastDt(self):
        self.assertEqual(
            self.postponement._past_datetime_from, datetimetz(1990, 10, 11, 13, 0)
        )
    ...
    def testCancellationView(self):
        ...
        self.assertEqual(
            details.string.strip(), "The meeting has been postponed until tomorrow"
        )
        toLink = select(".joy-ev-to-when a")[0]
        self.assertEqual(
            toLink.string.strip(), "Thursday 11th of October 1990 at 1pm to 4:30pm"
        )
        self.assertEqual(
            toLink["href"], "/events/test-meeting/1990-10-10-postponement/"
        )
...
class TestTZ(TestCase):
    ...
    def testLocalTitle(self):
        self.assertEqual(
            self.postponement.title, "Postponement for Wednesday 10th of October 1990"
        )
        self.assertEqual(
            self.postponement.local_title,
