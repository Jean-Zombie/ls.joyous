...
class Test(TestCase):
    ...
    def setUp(self):
        ...
        self.event = RecurringEventPage(
            ...
            title="Test Meeting",
            repeat=Recurrence(
                dtstart=dt.date(2010, 1, 1), freq=WEEKLY, byweekday=[MO, WE, FR]
            ),
    ...
    def testGetUpcomingEvents(self):
        event = RecurringEventPage(
            ...
            repeat=Recurrence(dtstart=dt.date(2021, 1, 1), freq=WEEKLY, byweekday=[FR]),
        ...
        shutdown.save_revision().publish()
        events = (
            ExtCancellationPage.events.exclude(cancellation_title="")
            .upcoming()
            .this()
            .descendant_of(event)
        )
    ...
    def testGetMyRawDates(self):
        dates = list(
            self.shutdown._getMyRawDates(dt.date(2020, 5, 30), dt.date(2020, 6, 10))
        )
        self.assertEqual(
            dates, [dt.date(2020, 5, 30), dt.date(2020, 5, 31), dt.date(2020, 6, 1)]
        )

    def testClosedOn(self):
        shutdown2 = ExtCancellationPage(
            owner=self.user,
            overrides=self.event,
    ...
    def testWhenEver(self):
        event = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(2020, 1, 1), freq=MONTHLY, byweekday=[FR(-1)]
            ),
            time_from=dt.time(19),
        )
        self.calendar.add_child(instance=event)
        shutdown = ExtCancellationPage(
            owner=self.user, overrides=event, cancelled_from_date=dt.date(2020, 4, 1)
        )
        event.add_child(instance=shutdown)
        shutdown.save_revision().publish()
        self.assertEqual(
    ...
    def testCurrentDt(self):
        self.assertEqual(
            self.shutdown._current_datetime_from, datetimetz(2020, 3, 25, 13, 0)
        )

    @freeze_timetz("2020-03-25 14:00")
    def testFutureDt(self):
        self.assertEqual(
            self.shutdown._future_datetime_from, datetimetz(2020, 3, 27, 13, 0)
        )

    @freeze_timetz("2020-03-25 14:00")
    def testPastDt(self):
        self.assertEqual(
            self.shutdown._past_datetime_from, datetimetz(2020, 3, 25, 13, 0)
        )
    ...
    def testUnexplainedCancellation(self):
        ...
        restriction = PageViewRestriction.objects.create(
            restriction_type=PASSWORD, password="s3cr3t", page=shutdown
        )
        restriction.save()
        events = RecurringEventPage.events.byDay(
...
class TestPageForm(TestCase):
    ...
    def testValid(self):
        ...
        form = self.Form(
            {"overrides": self.event, "cancelled_from_date": "2020-03-10"},
            instance=page,
    ...
    def testExceptionAlreadyExists(self):
        page = ExtCancellationPage(owner=self.user)
        form = self.Form(
            {"overrides": self.event, "cancelled_from_date": "2020-03-02"},
            instance=page,
            parent_page=self.event,
        )
