...
class TestVDt(TestCase):
    ...
    def testAwareDt(self):
        ...
        self.assertEqual(v, vDatetime.from_ical("20130425T060000", "Pacific/Chatham"))
...
class TestVEventFactory(TestCase):
    ...
    def testDtstartDtendTypes(self):
        ...
        with self.assertRaises(CalendarTypeError) as expected:
            self.factory.makeFromProps(props, None)
        self.assertEqual(
            str(expected.exception), "DTSTART and DTEND types do not match"
        )
    ...
    def testDiffTZsDtstartRecurrenceId(self):
        ...
        with self.assertRaises(CalendarTypeError) as expected:
            self.factory.makeFromProps(props, None)
        self.assertEqual(
            str(expected.exception), "DTSTART.timezone != RECURRENCE-ID.timezone"
        )
    ...
    def test1hrDuration(self):
        ...
        self.assertIs(type(vev), SimpleVEvent)
        self.assertEqual(
            vev["DTSTART"], timezone.make_aware(dt.datetime(2016, 6, 1, 7))
        )
