...
class Test(TestCase):
    ...
    def testServeSimple(self):
        ...
        response = self.handler.serve(
            page, self._getRequest("/events/baseball-game/" "?format=google")
        )
    ...
    def testServeMultiday(self):
        ...
        page.save_revision().publish()
        response = self.handler.serve(
            page, self._getRequest("/events/food-festival/" "?format=google")
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url,
    ...
    def testServeRecurring(self):
        page = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(2017, 3, 15), freq=MONTHLY, byweekday=[FR(-1)]
            ),
        ...
        page.save_revision().publish()
        response = self.handler.serve(
            page, self._getRequest("/events/lunch-and-code/" "?format=google")
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url,
    ...
    def testServeException(self):
        ...
        response = self.handler.serve(
            info,
            self._getRequest(
                "/events/test-meeting/1998-11-11/extra-info/" "?format=google"
            ),
        )
        self.assertEqual(response.status_code, 302)
        self.assertEqual(
