...
class Test(TestCase):
    ...
    def testGetEventsByDay(self):
        events = MultidayEventPage.events.byDay(
    ...
    def testWhen(self):
        self.assertEqual(
            self.event.when,
            "Monday 31st of December 2012 at 11pm to "
    ...
    def testPastDt(self):
        self.assertEqual(
            self.event._past_datetime_from, datetimetz(2012, 12, 31, 23, 0)
        )
        now = timezone.localtime()
        today = now.date()
        nextWeek = today + dt.timedelta(7 - today.weekday())
...
class TestTZ(TestCase):
    ...
    def testGetEventsByLocalDay(self):
        events = MultidayEventPage.events.byDay(
            dt.date(2018, 3, 1), dt.date(2018, 3, 31)
        )
        self.assertEqual(len(events), 31)
        evod1 = events[16]
    ...
    def testLocalWhen(self):
        self.assertEqual(
            self.event.when, "Friday 16th of March to Wednesday 21st of March"
        )
...
class TestPageForm(TestCase):
    ...
    def testEndDateBeforeStartDate(self):
        ...
        self.assertDictEqual(
            form.errors, {"date_to": ["Event cannot end before it starts"]}
        )
