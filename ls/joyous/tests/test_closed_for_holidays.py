...
from ls.joyous.models import RecurringEventPage, CancellationPage, ExtCancellationPage
...
class Test(TestCase):
    ...
    def testGetEventsByDay(self):
        events = RecurringEventPage.events.hols(self.calendar.holidays).byDay(
            dt.date(1989, 1, 1), dt.date(1989, 1, 31)
        )
    ...
    def testGetUpcomingEvents(self):
        event = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(1980, 1, 1), freq=MONTHLY, byweekday=[MO(1)]
            ),
        ...
        events = (
            ClosedForHolidaysPage.events.hols(self.calendar.holidays)
    ...
    def testGetPastEvents(self):
        events = ClosedForHolidaysPage.events.hols(self.calendar.holidays).past().this()
        self.assertEqual(len(events), 1)
        title, page, url = events[0]
    ...
    def testWombatGetEventsByDay(self):
        event = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(1989, 1, 1), freq=MONTHLY, byweekday=[MO(1)]
            ),
    ...
    def testGetMyDates(self):
        ...
        self.calendar.add_child(instance=event)
        closedHols = ClosedForHolidaysPage(
            owner=self.user, overrides=event, all_holidays=False
        )
        event.add_child(instance=closedHols)
        closedHols.save_revision().publish()
    ...
    def testCurrentDt(self):
        ...
        self.assertEqual(
            self.closedHols._current_datetime_from, datetimetz(1989, 3, 13, 13, 0)
        )
    ...
    def testFutureDt(self):
        # Taranaki Anniversary Day
        self.assertEqual(
            self.closedHols._future_datetime_from, datetimetz(1989, 3, 13, 13, 0)
        )
    ...
    def testPastDt(self):
        # Waitangi Day
        self.assertEqual(
            self.closedHols._past_datetime_from, datetimetz(1989, 2, 6, 13, 0)
        )
    ...
    def testFutureDtNoHolidays(self):
        ...
        self.assertEqual(
            self.closedHols._future_datetime_from, ClosedForHolidaysPage.MAX_DATETIME
        )
    ...
    def testPastDtNoHolidays(self):
        self.closedHols.holidays = None
        self.assertEqual(
            self.closedHols._past_datetime_from, ClosedForHolidaysPage.MIN_DATETIME
        )
    ...
    def testFutureDtCancelled(self):
        ...
        cancellation = CancellationPage(
            owner=self.user, overrides=self.event, except_date=dt.date(1989, 3, 13)
        )
        self.event.add_child(instance=cancellation)
        self.assertEqual(
            self.closedHols._future_datetime_from, datetimetz(1989, 3, 20, 13, 0)
        )
    ...
    def testPastDtCancelled(self):
        # a cancellation page trumps the holiday
        cancellation = CancellationPage(
            owner=self.user, overrides=self.event, except_date=dt.date(1989, 2, 6)
        )
        self.event.add_child(instance=cancellation)
        self.assertEqual(
            self.closedHols._past_datetime_from, datetimetz(1989, 1, 30, 13, 0)
        )
    ...
    def testPastDtShut(self):
        ...
        self.assertEqual(
            self.closedHols._past_datetime_from, datetimetz(1989, 1, 23, 13, 0)
        )
    ...
    def testFutureDtShut(self):
        ...
        self.event.add_child(instance=shutdown)
        self.assertEqual(
            self.closedHols._future_datetime_from, datetimetz(1989, 3, 24, 13, 0)
        )
    ...
    def testFarFutureDt(self):
        event = RecurringEventPage(
            slug="UVW",
            title="Underwater viking wombats",
            repeat=Recurrence(dtstart=dt.date(1989, 1, 1), freq=MONTHLY, bymonthday=23),
            time_from=dt.time(19),
        )
        self.calendar.add_child(instance=event)
        closedHols = ClosedForHolidaysPage(
        ...
        self.assertEqual(
            closedHols._future_datetime_from, datetimetz(9999, 12, 29, 19, 0)
        )
...
class TestPageForm(TestCase):
    ...
    def setUp(self):
        ...
        self.event = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(2017, 1, 1), freq=MONTHLY, byweekday=[MO(1), MO(3)]
            ),
            time_from=dt.time(13),
            time_to=dt.time(15, 30),
            holidays=holidays,
    ...
    def testValid(self):
        page = ClosedForHolidaysPage(owner=self.user, holidays=self.event.holidays)
        form = self.Form(
            {
                "overrides": self.event,
                "all_holidays": False,
        ...
        self.assertCountEqual(saved.closed, ["St Swithin's Day", "Sleepy Head Day"])

    def testDefaultHolidayChoices(self):
        page = ClosedForHolidaysPage(owner=self.user)
        form = self.Form(
            {"overrides": self.event}, instance=page, parent_page=self.event
        )
        self.assertCountEqual(
            form.fields["closed_for"].choices,
            [
