...
class Test(TestCase):
    ...
    def testGetEventsByDay(self):
        events = SimpleEventPage.events.byDay(dt.date(1987, 6, 1), dt.date(1987, 6, 30))
    ...
    def testStatus(self):
        ...
        futureEvent = SimpleEventPage(
            owner=self.user, slug="tomorrow", title="Tomorrow's Event", date=tomorrow
        )
    ...
    def testCurrentDt(self):
        ...
        self.calendar.add_child(instance=futureEvent)
        self.assertEqual(
            futureEvent._current_datetime_from, datetimetz(tomorrow, dt.time.max)
        )
    ...
    def testFutureDt(self):
        ...
        self.calendar.add_child(instance=futureEvent)
        self.assertEqual(
            futureEvent._future_datetime_from, datetimetz(tomorrow, dt.time.max)
        )
    ...
    def testGroup(self):
        ...
        race = SimpleEventPage(
            owner=self.user, slug="race", title="Race", date=dt.date(2008, 6, 3)
        )
...
class TestTZ(TestCase):
    ...
    def testGetEventsByLocalDay(self):
        evods = SimpleEventPage.events.byDay(dt.date(1987, 6, 1), dt.date(1987, 6, 30))
    ...
    def testGetEventsAcrossDateline(self):
        ...
        evods = SimpleEventPage.events.byDay(dt.date(2016, 7, 31), dt.date(2016, 7, 31))
...
class TestQuerySet(TestCase):
    ...
    def testByDay(self):
        evods = SimpleEventPage.events.byDay(dt.date(2015, 6, 5), dt.date(2015, 6, 5))
    ...
    def testPasswordAuth(self):
        ...
        restriction = PageViewRestriction.objects.create(
            restriction_type=PASSWORD, password="s3cr3t", page=meeting
        )
        self.assertEqual(list(SimpleEventPage.events.all()), [self.event, meeting])
        request = RequestFactory().get("/test")
        request.user = self.user
        request.session = {}
        self.assertEqual(list(SimpleEventPage.events.auth(request)), [self.event])
        request.session[KEY] = [restriction.id]
        self.assertEqual(
            list(SimpleEventPage.events.auth(request)), [self.event, meeting]
        )
    ...
    def testLoginAuth(self):
        ...
        self.assertEqual(list(SimpleEventPage.events.auth(request)), [self.event])
        request.user = self.user
        self.assertEqual(list(SimpleEventPage.events.auth(request)), [self.event, bee])
