...
class Test(TestCase):
    def setUp(self):
        ...
        self.standup = RecurringEventPage(
            slug="test-meeting",
            title="Test Meeting",
            repeat=Recurrence(
    ...
    def testAuthGetAllEventsByDay(self):
        ...
        events = getAllEventsByDay(
            self.request, dt.date(2013, 1, 1), dt.date(2013, 1, 31)
        )
        self.assertEqual(len(events), 31)
        evod10 = events[9]
        self.assertEqual(evod10.date, dt.date(2013, 1, 10))
    ...
    def testGetAllUpcomingEvents(self):
        ...
        events = getAllUpcomingEvents(
            self.request, home=self.home, holidays=self.calendar.holidays
        )
    ...
    def testGetGroupUpcomingEvents(self):
        meeting = RecurringEventPage(
            owner=self.user,
            slug="plan-plan",
            title="Planning to Plan",
            repeat=Recurrence(dtstart=dt.date(2018, 5, 1), freq=WEEKLY, byweekday=[TU]),
    ...
    def testGetGroupUpcomingEventsDupGroup(self):
        meeting = RecurringEventPage(
            ...
            slug="plan-plan",
            title="Planning to Plan",
            repeat=Recurrence(dtstart=dt.date(2018, 5, 2), freq=WEEKLY, byweekday=[WE]),
            time_from=dt.time(18, 30),
            time_to=dt.time(20),
            group_page=self.group,
        )
    ...
    def testAuthGetEventFromUid(self):
        with self.assertRaises(PermissionDenied):
            event = getEventFromUid(
                self.request, "80af64e7-84e6-40d9-8b4f-7edf92aab9f7"
            )
        self.request.user.groups.set([self.friends])
...
class TestTZ(TestCase):
    def setUp(self):
        ...
        self.night = RecurringEventPage(
            slug="pacnight",
            title="Pacific Night",
            repeat=Recurrence(
                dtstart=dt.date(2018, 12, 1), count=1, freq=MONTHLY, byweekday=[SU(-1)]
            ),
    ...
    def testExtremeTZGetAllEventsByDay(self):
        events = getAllEventsByDay(
            self.request, dt.date(2019, 1, 1), dt.date(2019, 1, 31)
        )
        self.assertEqual(len(events), 31)
        evod1 = events[0]
