...
class TestSimple(TestCase):
    ...
    def testFromPage(self):
        ...
        tz = pytz.timezone("Australia/Sydney")
        self.assertEqual(vev["DTSTART"].dt, tz.localize(dt.datetime(1987, 6, 5, 11, 0)))
...
class TestMultiday(TestCase):
    ...
    def testFromPage(self):
        ...
        tz = pytz.timezone("Pacific/Niue")
        self.assertEqual(vev["DTSTART"].dt, tz.localize(dt.datetime(2018, 3, 16, 0, 0)))
        self.assertEqual(
...
class TestRecurring(TestCase):
    ...
    def testExdate(self):
        page = RecurringEventPage(
            ...
            title="Sleep In",
            repeat=Recurrence(
                dtstart=dt.date(2018, 5, 1), freq=MONTHLY, byweekday=[SA(+2)]
            ),
            time_from=dt.time(7),
...
class TestMultidayRecurring(TestCase):
    ...
    def testFromPage(self):
        page = MultidayRecurringEventPage(
            ...
            repeat=Recurrence(dtstart=dt.date(2019, 4, 2), freq=WEEKLY, byweekday=[TU]),
            num_days=3,
        ...
        tz = pytz.timezone("Pacific/Auckland")
        self.assertEqual(vev["DTSTART"].dt, tz.localize(dt.datetime(2019, 4, 2, 16, 0)))
        self.assertEqual(vev["DTEND"].dt, tz.localize(dt.datetime(2019, 4, 4, 18, 0)))
