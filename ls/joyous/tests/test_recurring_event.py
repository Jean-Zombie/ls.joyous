...
from ls.joyous.models import RecurringEventPage, CancellationPage, PostponementPage
...
class Test(TestCase):
    def setUp(self):
        ...
        self.event = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(2017, 8, 5), freq=MONTHLY, byweekday=[TU(1)]
            ),
    ...
    def testGetEventsByDay(self):
        events = RecurringEventPage.events.byDay(
            dt.date(2017, 8, 1), dt.date(2017, 10, 31)
        )
    ...
    def testStatus(self):
        ...
        nowEvent = RecurringEventPage(
            owner=self.user,
            slug="now",
            title="Now Event",
            repeat=Recurrence(dtstart=dt.date(2010, 1, 1), freq=DAILY),
        ...
        notToday = [
            weekday for weekday in EVERYWEEKDAY if weekday.weekday != today.weekday()
        ]
        pastAndFutureEvent = RecurringEventPage(
            ...
            repeat=Recurrence(
                dtstart=dt.date(2009, 8, 7), freq=WEEKLY, byweekday=notToday
            ),
        )
    ...
    def testWhen(self):
        self.assertEqual(
            self.event.when, "The first Tuesday of the month at 6:30pm to 8pm"
        )
    ...
    def testCurrentDt(self):
        ...
        movieNight = RecurringEventPage(
            owner=self.user,
            slug="movies",
            title="Movies",
            repeat=Recurrence(
                dtstart=dt.date(2005, 2, 1),
                freq=WEEKLY,
                byweekday=[
                    TH,
                ],
        ...
        self.assertEqual(
            movieNight._current_datetime_from, datetimetz(thursday, startTime)
        )
    ...
    def testFutureDt(self):
        ...
        thursday = myday + dt.timedelta(days=(3 - myday.weekday()) % 7)
        self.assertEqual(
            movieNight._future_datetime_from, datetimetz(thursday, startTime)
        )
    ...
    def testPastDt(self):
        ...
        self.assertEqual(
            movieNight._past_datetime_from, datetimetz(thursday, startTime)
        )
...
class TestTZ(TestCase):
    def setUp(self):
        ...
        self.event = RecurringEventPage(
            ...
            repeat=Recurrence(dtstart=dt.date(2017, 1, 1), freq=WEEKLY, byweekday=[TU]),
    ...
    def testGetEventsByLocalDay(self):
        events = RecurringEventPage.events.byDay(
            dt.date(2018, 4, 1), dt.date(2018, 4, 30)
        )
    ...
    def testExtremeTimeZones(self):
        lions = RecurringEventPage(
            owner=self.user,
            slug="pago-pago-lions",
            title="Pago Pago Lions Club",
            repeat=Recurrence(
                dtstart=dt.date(2015, 2, 1), freq=MONTHLY, byweekday=[TH(1), TH(3)]
            ),
